...
const getStatusStyles = (status: string) => {
  switch (status.toLowerCase()) {
    ...
    case 'interview':
      return 'bg-purple-500/12 text-purple-300 border border-purple-500/25';
    case 'review':
      return 'bg-amber-500/12 text-amber-300 border border-amber-500/25';
    case 'screening':
      return 'bg-cyan-500/12 text-blue-300 border border-blue-500/25';
    default:
    ...
    <main className="relative min-h-screen overflow-x-hidden bg-[#0A0A0F] px-3 py-5 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <section className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
          {KPI_DATA.map((item) => {
            ...
            return (
              <div
                key={item.label}
                className="group relative overflow-hidden rounded-3xl border border-cyan-400/20 bg-white/[0.03] p-6 backdrop-blur-2xl shadow-[0_0_20px_rgba(6,182,212,0.08)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_25px_rgba(6,182,212,0.14)]"
              >
                ...
                <h2 className="relative mt-5 text-4xl font-bold text-cyan-200">
                  {item.value}
                </h2>
              </div>
            );
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          {/* Chart */}
          <div className="lg:col-span-2 rounded-[2rem] border border-white/10 bg-white/[0.03] p-6 backdrop-blur-xl">
            ...
            <div className="mt-6 flex flex-col gap-4">
              {[
                ...
                <Link
                  key={action.label}
                  href={action.href}
                  className="flex items-center justify-between rounded-2xl border border-cyan-400/20 bg-cyan-500/10 px-4 py-4 backdrop-blur-xl shadow-[0_0_18px_rgba(6,182,212,0.05)] hover:bg-cyan-500/15 hover:shadow-[0_0_25px_rgba(6,182,212,0.12)] transition-all duration-300"
                >
        ...
        {/* Recent Applications */}
        <section className="rounded-4xl border border-white/10 hover:bg-cyan-500/5 transition-colors duration-300 p-6 backdrop-blur-xl overflow-x-auto no-scrollbar">
          <h2 className="text-2xl font-bold text-cyan-300">
            Recent Applications
          </h2>
        ...
        <section className="rounded-[2rem] border border-white/10 bg-white/[0.03] p-6 backdrop-blur-xl">
          ...
          <div className="mt-6 grid gap-4">
            {activeJobs.map((job) => (
              <div
                key={job.id}
                className="rounded-3xl border border-cyan-500/15 bg-white/[0.03] p-5 backdrop-blur-xl shadow-[0_0_18px_rgba(6,182,212,0.04)] flex flex-col md:flex-row md:items-center md:justify-between gap-4 hover:border-cyan-400/25 transition-all duration-300"
              >
                <div>
                ...
                <div className="flex gap-3">
                  <Link
                    ...
                    className="rounded-xl border border-cyan-500/20 px-4 py-2 text-cyan-300"
                  >
                    Edit
                  </Link>
                  ...
                  <Link
                    href={`/jobs/${job.id}`}
                    className="rounded-xl bg-cyan-500/15 px-4 py-2 text-cyan-200"
                  >
                    View
                  </Link>
                </div>
